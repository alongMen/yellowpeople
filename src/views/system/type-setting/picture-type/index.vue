...
import {
  ...
  add_Picture_type_list,
  update_Picture_type_list,
  delete_Picture_type_list
} from '@/service/index';
...
export default {
  ...
  methods: {
    ...
    async del(row,index) {
      this.$Modal.confirm({
        ...
        onOk: async () => {
          ...
          const res = await delete_Picture_type_list(row.id);
    ...
    async ok() {
      ...
      if (this.form.id) {
        this.btnLoading = true;
        const res = await update_Picture_type_list(this.form);
        this.btnLoading = false;
        if (res.status < 400) {
        ...
        this.btnLoading = true;
        const res = await add_Picture_type_list(this.form);
        this.btnLoading = false;
        if (res.status < 400) {
