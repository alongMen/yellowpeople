...
import {
  get_video_type_list,
  add_video_type_list,
  update_video_type_list,
  delete_video_type_list
} from '@/service/index';
...
export default {
  ...
  methods: {
    ...
    async del(row,index) {
      this.$Modal.confirm({
        ...
        onOk: async () => {
          ...
          const res = await delete_video_type_list(row.id);
    ...
    async getData() {
      this.loading = true;
      const res = await get_video_type_list(this.query);
    ...
    async ok() {
      ...
      if (this.form.id) {
        this.btnLoading = true;
        const res = await update_video_type_list(this.form);
        this.btnLoading = false;
        if (res.status < 400) {
        ...
        this.btnLoading = true;
        const res = await add_video_type_list(this.form);
        this.btnLoading = false;
        if (res.status < 400) {
